	...
	function CreateDB(dbName,alias){ //set-up database name	  
         ...
         mydb = openDatabase(dbName, '1.0', alias, 2 * 1024 * 1024);     
		 console.log ('Finished Creating database');		 
	 }
	function CreateTable(tblname,fileds){ //create a table
		mydb.transaction(function (txs) {
		//txs.executeSql("CREATE TABLE IF NOT EXISTS '"+ tblname +"'(id unique, name TEXT)");
		txs.executeSql("CREATE TABLE IF NOT EXISTS '"+ tblname +"'" + fields);
        });
	 }	
   function InsertTable(tblname,fileds,values){//Insert into a table
		mydb.transaction(function (txs) {
      //  txs.executeSql("DELETE FROM '"+tblname +"' WHERE userID='"+ userID +"'");	
		//txs.executeSql("INSERT INTO '"+ tblname +"' (userID,data) VALUES('"+ userID +"','" +data +"')");  
		txs.executeSql("INSERT INTO '"+ tblname +"'"+ fields + values); 		
         });
	 }	
